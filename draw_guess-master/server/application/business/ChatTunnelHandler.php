...
class ChatTunnelHandler implements ITunnelHandler {
  ...
  /**
    ...
    */
  private static function broadcast($type, $content, $mask=false, $senderTunnelId=0) {
      $data = self::loadData();
      $tunnelIds = array();
      if(!$mask){
        $tunnelIds = $data['connectedTunnelIds'];
      }
      else{
        foreach ($data['connectedTunnelIds'] as $tunnelId){
          if($senderTunnelId === $tunnelId){
            continue;
          }else{
            array_push($tunnelIds, $tunnelId);
          }
        }
      }
